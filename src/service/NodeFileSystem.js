...
export class NodeFileSystem {
  ...
  async loadBinaryModel(): Promise<tfc.io.ModelArtifacts> {
    const topologyPath = this.path[0];
    const weightManifestPath = this.path[1];
    const topology = await stat(topologyPath).catch(
  ...
  async loadJSONModel(): Promise<tfc.io.ModelArtifacts> {
